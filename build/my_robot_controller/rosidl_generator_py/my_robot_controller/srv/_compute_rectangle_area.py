...
class ComputeRectangleArea_Response(metaclass=Metaclass_ComputeRectangleArea_Response):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.area_response = kwargs.get('area_response', float())
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.area_response != other.area_response:
            return False
        return True
    ...
    @builtins.property
    def area_response(self):
        """Message field 'area_response'."""
        return self._area_response

    @area_response.setter
    def area_response(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'area_response' field must be of type 'float'"
            assert not (value < -1.7976931348623157e+308 or value > 1.7976931348623157e+308) or math.isinf(value), \
                "The 'area_response' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._area_response = value
...
class Metaclass_ComputeRectangleArea(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'my_robot_controller.srv.ComputeRectangleArea')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from my_robot_controller.srv import _compute_rectangle_area
...
class ComputeRectangleArea(metaclass=Metaclass_ComputeRectangleArea):
    from my_robot_controller.srv._compute_rectangle_area import ComputeRectangleArea_Request as Request
    from my_robot_controller.srv._compute_rectangle_area import ComputeRectangleArea_Response as Response
